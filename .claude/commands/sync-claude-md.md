Check that every file path and exported symbol quoted in CLAUDE.md still exists in the codebase. Fix any that are stale.
...
5. For each stale reference found, update the corresponding line in `CLAUDE.md` to match the current codebase. If a file was deleted with no replacement, remove or reword the sentence that referenced it.
